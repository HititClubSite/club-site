...
app.get('/api/visitor',async(req,res)=>{const ip=(req.headers['x-forwarded-for']||req.socket.remoteAddress||'').split(',')[0].trim().replace('::ffff:','');let location='YaklaÅŸÄ±k konum bulunamadÄ±';try{const r=await fetch(`http://ip-api.com/json/${encodeURIComponent(ip)}?fields=status,city,country,query`);const d=await r.json();if(d.status==='success')location=[d.city,d.country].filter(Boolean).join(', ')}catch{}const logs=await read(logsFile);logs.unshift({ip,location,time:new Date().toISOString()});await write(logsFile,logs.slice(0,100));res.json({ip,location})});
app.get('/admin',(req,res)=>{if(!req.session.admin)return res.send(page('Admin giriÅŸ',`<h1>YÃ¶netim paneli</h1><form method="post" action="/admin/login"><input type="password" name="password" placeholder="Admin ÅŸifresi" required autofocus><button>GiriÅŸ yap</button>${req.query.error?'<p class="error">Åifre hatalÄ±.</p>':''}</form>`));res.redirect('/admin/panel')});
...
app.get('/admin/panel',admin,async(req,res)=>{const s=await read(settingsFile),logs=await read(logsFile);res.send(page('Admin panel',`<div class="head"><h1>Club Site / Admin</h1><form method="post" action="/admin/logout"><button>Ã‡Ä±kÄ±ÅŸ</button></form></div><form class="settings" method="post" action="/admin/settings"><label>Logo URL veya dosya yolu<input name="logo" value="${s.logo}"></label><label>WhatsApp baÄŸlantÄ±sÄ±<input name="whatsappUrl" type="url" value="${s.whatsappUrl}"></label><label>KarÅŸÄ±lama mesajÄ± <small>{location} konumla deÄŸiÅŸtirilir</small><textarea name="welcomeTemplate">${s.welcomeTemplate}</textarea></label><button>Kaydet</button></form><h2>ZiyaretÃ§i loglarÄ±</h2><table><tr><th>IP</th><th>YaklaÅŸÄ±k konum</th><th>Zaman</th></tr>${logs.map(x=>`<tr><td>${x.ip}</td><td>${x.location}</td><td>${new Date(x.time).toLocaleString('tr-TR')}</td></tr>`).join('')}</table>`))});
app.post('/admin/settings',admin,async(req,res)=>{await write(settingsFile,{logo:req.body.logo||'/logo.png',whatsappUrl:req.body.whatsappUrl||'#',universityUrl:req.body.universityUrl||'#',welcomeTemplate:req.body.welcomeTemplate||'',events:JSON.parse(req.body.events||'[]')});res.redirect('/admin/panel')});
